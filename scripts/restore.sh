...
# Best-effort ownership fix. Will succeed if running as root, may no-op when
# running as the mysql user — in which case the file ownership from --copy-back
# already matches mysql:mysql since the source files were owned that way.
chown -R mysql:mysql "${TARGET_DIR}" 2>/dev/null || true
